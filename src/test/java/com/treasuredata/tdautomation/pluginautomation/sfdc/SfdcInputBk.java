...
@Listeners(SfdcInputBk.class)

public class SfdcInputBk extends TestCaseBase {
    ...
    public static String YMLv2 = Constant.RESOURCE_PATH + "pluginautomation/sfdc/yml/replaceMode.yml";

    public static String CSV_SOURCE_TEST_DATA = String.format("%spluginautomation/sfdc/csv/basic.csv", Constant.RESOURCE_PATH);
    public static String CSV_TARGET_TEST_DATA = String.format("%spluginautomation/sfdc/csv/target.csv", Constant.RESOURCE_PATH);

    public static String TD_QUERY_TABLE = String.format("select %s__c as %s, %s__c as %s, %s__c as %s, %s__c as %s, %s__c as %s, TD_TIME_FORMAT(TD_TIME_PARSE(%s__c), 'HH:mm:ss') as %s from %s",
    ...
    public void initialize(@Optional("csv file") String csvFile, @Optional("yml file") String ymlFile){
        LOGGER = LogManager.getLogger(SfdcInputBk.class.getName());
        TD_DB = TD_USER + "_sfdc";
    ...
    @Test(dependsOnMethods = {"callDataTransferToTdV2"}, retryAnalyzer = TestCaseBase.class)
